...
private[spark] class ApplicationInfo(
  ...
  /**
    * requestedCores  就是默认当前application启动要使用多少cores
    * 如果提交任务时指定了 --total-executor-core 指定了当前application最多使用的core是多少，那么就是指定的值
    */
  private val requestedCores = desc.maxCores.getOrElse(defaultCores)
  //coresGranted 已经分配的core ,刚开始就是0个，假设requestedCores 是最大值
